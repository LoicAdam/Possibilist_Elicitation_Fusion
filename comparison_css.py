# -*- coding: utf-8 -*-
"""
Compare strategies using CSS elicitation.
"""
import pickle
import os
...
if __name__ == '__main__':
    ...
    rational = d['rational']
    elicitation_classic = d['elicitation_classic'] 
    ...
    mmr_real_classic = np.zeros(nb_repetitions)
    ...
    for i in range(0, nb_repetitions):
        ...
        mmr_real_classic[i] = elicitation_classic[i]['mmr_real'][-1]
    ...
    plt.xticks([1, 2, 3, 4, 5, 6, 7], ['classic', 'zero', 'kn zero', 'mcs zero', 
                                       'max', 'kn max', 'mcs max'])
    ax.set_ylim(-0.05,1)
    fig.set_dpi(300.0)
    ...
    data_inconsistency_zero = [mmr_real_classic[inconsistency_before_zero == 0],
                              mmr_real_zero[inconsistency_before_zero == 0],
                              mmr_real_k_n_zero[inconsistency_before_zero == 0],
                              mmr_mcs_zero[inconsistency_before_zero == 0],
                              mmr_real_maximum[inconsistency_before_maximum == 0],
    ...
    data_inconsistency_low = [mmr_real_classic[(0 < inconsistency_before_zero) * (inconsistency_before_zero <= inconsistency_threshold)],
                              mmr_real_zero[(0 < inconsistency_before_zero) * (inconsistency_before_zero <= inconsistency_threshold)],
    ...
    data_inconsistency_high = [mmr_real_classic[inconsistency_before_zero > inconsistency_threshold],
                              mmr_real_zero[inconsistency_before_zero > inconsistency_threshold],
    ...
    data = np.asarray([mmr_real_classic, mmr_real_zero, mmr_real_k_n_zero,
                       mmr_mcs_zero, mmr_real_maximum, mmr_real_k_n_maximum,
                       mmr_mcs_maximum]).T
    mat_better = np.zeros((7,7))
    mat_strictly_better = np.zeros((7,7))
